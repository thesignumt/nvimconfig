...
function Cursor:set(index, opts)
    ...
    local opts = vim.tbl_extend('force', { silent = false, setdefault = false }, opts or {})
...
function Cursor:_save_default()
    ...
    local ok, write_err = pcall(vim.uv.fs_write, fd, tostring(self.default_index), -1)

    vim.uv.fs_close(fd) -- always close

    if not ok then
        vim.notify('Failed to write cursor default: ' .. (write_err or ''), vim.log.levels.ERROR)
    end
end
