...
local function start_record(hq)
    ...
    local base = fpath('~/Videos/nvim', ('%s(%s)%s'):format(current_file, time, suffix))
    ...
    local args = {
        ...
        '60',
        '-filter:v',
        hq and 'crop=1920:1080:(in_w-1920)/2:(in_h-1080)/2,unsharp=7:7:1.0:7:7:0.0' or 'crop=1920:1080:(in_w-1920)/2:(in_h-1080)/2,unsharp=5:5:1.0:5:5:0.0',
        '-c:v',
        'libx264',
        '-pix_fmt',
        'yuv420p',
    ...
    vim.schedule(function()
        vim.notify((hq and 'high quality ' or '') .. 'recording to: ' .. full_path, vim.log.levels.INFO)
    end)
end
...
local function aesthetic_screenshot()
    ...
    local current_file = vim.fn.expand '%:t'
    local time = os.date '%H-%M'
    local base = fpath('~/Videos/nvimPhotos', ('%s(%s)'):format(current_file, time))
